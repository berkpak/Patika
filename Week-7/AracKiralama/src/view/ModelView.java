...
public class ModelView extends Layout {
    ...
    public ModelView(Model model) {
        ...
        if(this.model.getId() != 0){
            ...
            ComboItem defaulBrand = new ComboItem(this.model.getBrand().getId(), this.model.getBrand().getName());
            this.cmb_brand.getModel().setSelectedItem(defaulBrand);
        }

        this.btn_model_save.addActionListener(e -> {
            if (Helper.isFiledListEmpty(new JTextField[]{this.fld_model_name, this.fld_model_year})) {
                Helper.showMsg("fill");
            } else {
